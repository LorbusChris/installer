...
func (a *Bootstrap) addStorageFiles(base string, uri string, templateData *bootstrapTemplateData) (err error) {
	...
	replaceOrAppend(a.Config.Storage.Files, ign)
...
func (a *Bootstrap) addParentFiles(dependencies asset.Parents) {
	...
	for _, asset := range []asset.WritableAsset{
		...
		for _, file := range ignition.FilesFromAsset(rootDir, "root", 0644, asset) {
			replaceOrAppend(a.Config.Storage.Files, file)
		}
	}
	...
	replaceOrAppend(a.Config.Storage.Files, ignition.FileFromBytes(filepath.Join(rootDir, rootCA.CertFile().Filename), "root", 0644, rootCA.Cert()))
}

func replaceOrAppend(files []igntypes.File, file igntypes.File) {
	for i, f := range files {
